...
For 4POLAR: The upper drop down menu lists all the calibration data included in the app. If the calibration data to be used is not in the list, select `other`and download the appropriate calibration data. The calibration data is a `.mat` file with a name of the type `Calib*.txt` containing a 4x3 matrix *K* for `4POLAR 2D` (see [template](https://github.com/cchandre/Polarimetry/blob/master/pypolar/calibration/Calib_th_2D.txt)) or a 4x4 matrix *K* for `4POLAR 3D` (see [template](https://github.com/cchandre/Polarimetry/blob/master/pypolar/calibration/Calib_th_3D.txt)). The name of the calibration data used in the analysis is displayed on the lower part of the panel.
