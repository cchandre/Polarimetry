...
from matplotlib import font_manager as m_font_manager
...
from pypolar_classes import default_fontname, default_fontsize, header_fontsize, button_size, geometry_info, default_cr
...
class Polarimetry(CTk.CTk):
    ...
    def show_about(self):
        ...
        tk.Label(about_win, text=info_text, font=('Arial Rounded MT Bold', 11), justify="center").pack(pady=10)
    ...
    def load_fonts(self):
        ...
        variants = ["Nunito-Regular.ttf", "Nunito-Medium.ttf", "Nunito-Bold.ttf", "Nunito-Italic.ttf"]
        ...
        for variant in variants:
            ...
            if font_path.exists():
                ...
                self.nunito_family_name = prop.get_name()
            else:
                print(f"Warning: Could not find {variant}")
        plt.rcParams['font.sans-serif'] = [self.nunito_family_name, 'sans-serif']
        plt.rcParams['font.family'] = 'sans-serif'
...
## RIGHT FRAME: INTENSITY
        ...
        entry = CTk.CTkEntry(banner, textvariable=self.pixel_size, border_color=gray[0], width=50, justify='center', font=(default_fontname, default_fontsize, "bold"), corner_radius=default_cr)
...
## RIGHT FRAME: THRSH
        ...
        entry = CTk.CTkEntry(master=ilow_frame, textvariable=self.ilow, border_color=gray[1], width=100, justify=CTk.LEFT, font=(default_fontname, default_fontsize, "bold"), corner_radius=default_cr)
...
## RIGHT FRAME: OPTIONS
        ...
        Label(master=show_save, text='Figures\n', width=250, fontsize=header_fontsize).grid(row=0, column=0, columnspan=3, padx=20, pady=(10, 0), sticky="n")
        ...
        Label(master=preferences, text='Preferences\n', width=250, fontsize=header_fontsize).grid(row=0, column=0, columnspan=2, padx=20, pady=(10, 0), sticky='n')
        ...
        Label(master=save_ext, text='Save output\n', width=250, fontsize=header_fontsize).grid(row=0, column=0, columnspan=2, padx=(20, 20), pady=(10, 0), sticky="n")
...
## RIGHT FRAME: ADVANCED
        ...
        for loc, elt, cspan in zip(adv_loc, adv_elts, columnspan):
            ...
            Label(master=adv[elt], text=elt + '\n', width=230, fontsize=header_fontsize).grid(row=0, column=0, columnspan=cspan, padx=20, pady=(10, 0), sticky="n")
        ...
        for _ in range(len(labels)):
            ...
            entry = CTk.CTkEntry(adv['Rotation'], textvariable=self.rotation[_], width=50, justify='center', font=(default_fontname, default_fontsize, "bold"), corner_radius=default_cr)
        ...
        CTk.CTkEntry(adv['Intensity removal'], textvariable=self.noise[0], width=50, justify='center', font=(default_fontname, default_fontsize, "bold"), corner_radius=default_cr).grid(row=4, column=0, sticky='e', padx=23)
...
## RIGHT FRAME: ABOUT
        button_frame = CTk.CTkFrame(self.tabview.tab('About'), fg_color="transparent")
        button_frame.pack(fill="x", pady=(30, 10), padx=50)
        btn_kwargs = {'side': 'left', 'padx': 30}

        Button(button_frame, image=self.icons['web'], 
            command=lambda: self.openweb(type(self).url_fresnel), 
            tooltip=' visit the polarimetry website').pack(**btn_kwargs)

        Button(button_frame, image=self.icons['mail'], 
            command=self.send_email, 
            tooltip=' send an email to report bugs').pack(**btn_kwargs)

        Button(button_frame, image=self.icons['GitHub'], 
            command=lambda: self.openweb(type(self).url_github), 
            tooltip=' visit the PyPOLAR GitHub page').pack(**btn_kwargs)

        Button(button_frame, image=self.icons['pdf'], 
            command=lambda: self.openweb(type(self).url_pdf), 
            tooltip=' download documentation').pack(**btn_kwargs)

        Button(button_frame, image=self.icons['contact_support'], 
            command=lambda: self.openweb(type(self).url_wiki), 
            tooltip=' visit the PyPOLAR wiki').pack(**btn_kwargs)
    ...
    def edge_button_callback(self) -> None:
        ...
        if hasattr(self, 'edge_contours'):
            ...
            if not hasattr(self, 'edge_window'):
                ...
                self.edge_window = ShowInfo(message=' Image for edge detection', image=self.icons['multiline_chart'], button_labels=['Download', 'Compute', 'Cancel'], geometry=(370, 140), fontsize=18)
    ...
    def edge_detection_tab(self) -> None:
        ...
        for elt, side in zip(adv_elts, adv_sides):
            ...
            Label(master=adv[elt], text=elt + '\n', width=230, fontsize=header_fontsize).grid(row=0, column=0, padx=20, pady=(10,0))
    ...
    def create_folder_query_widgets(self, parent, row, label_text, entry_variable_name):
        ...
        entry = CTk.CTkEntry(parent, textvariable=string_var, width=250, font=(default_fontname, default_fontsize, "bold"), corner_radius=default_cr)
    ...
    def crop_figures_callback(self) -> None:
        if len(plt.get_fignums()) and (not hasattr(self, 'crop_window')):
            ...
            Label(self.crop_window, text='  define xlim and ylim', image=self.icons['crop'], compound='left', fontsize=header_fontsize, width=200).grid(row=0, column=0, columnspan=2, padx=30, pady=(30, 5))
            self.crop_all_figs = CTk.BooleanVar(value=False)
            self.crop_check = CheckBox(self.crop_window, text="for all", fontsize=header_fontsize, variable=self.crop_all_figs, tooltip=" apply cropping to all figures instead of only those corresponding to the current stack")
            ...
            for _, label in enumerate(labels):
                Label(master=self.crop_window, text=label, fontsize=header_fontsize).grid(row=_+1, column=0, padx=(20, 0), pady=0)
        ...
        if event.inaxes == self.intensity_axis:
            ...
            if event.button == 1:
                if not roi.lines:
                    ...
                    ShowInfo(message=message, image=self.icons['square'], button_labels = ['OK'], fontsize=default_fontsize)
                    self.intensity_pyfig.canvas.draw()
                    self.compute_angle_button.configure(fg_color=orange[0])
                    self.intensity_pyfig.canvas.mpl_disconnect(self._cid1)
                    self.intensity_pyfig.canvas.mpl_disconnect(self._cid2)
    ...
    def define_variable_table(self, method:str) -> None:
        ...
        Label(master=self.variable_table_frame, text='Variables\n', width=250, fontsize=header_fontsize).grid(row=0, column=0, columnspan=4, padx=(20, 20), pady=(10, 0))
        ...
        for _, (var, val) in enumerate(zip(variables, vals)):
            ...
            CTk.CTkCheckBox(master=self.variable_table_frame, text=None, variable=self.variable_display[_], width=30, corner_radius=default_cr).grid(row=_+2, column=0, padx=(20, 0), sticky='w')
    ...
    def method_dropdown_callback(self, method:str) -> None:
        ...
        if method.startswith('4POLAR'):
            ...
            window = ShowInfo(message='\n Perform: Select a beads file (*.tif)\n\n Load: Select a registration file (*.pyreg)\n\n Registration is performed with Whitelight.tif \n   which should be in the same folder as the beads file', image=self.icons['blur_circular'], button_labels=['Perform', 'Load', 'Cancel'], geometry=(460, 270))
            buttons = window.get_buttons()
    ...
    def change_registration_callback(self, window:CTk.CTkToplevel, ims, beadstack) -> None:
        ...
        CTk.CTkLabel(window, text=" Enter new parameters for the registration", image=self.icons['blur_circular'], compound='left', width=250, justify=CTk.LEFT).grid(row=0, column=0, padx=30, pady=10)
        buttons = []
    ...
    def add_roi_button_press_callback(self, event:MouseEvent, roi:ROI) -> None:
        if event.inaxes == self.thrsh_axis:
            ...
            if (event.button == 1 or event.button == 3) and not event.dblclick:
                ...
                self.thrsh_axis.add_line(roi.lines[-1])     
                window = ShowInfo(message=' Add ROI?', image=self.icons['roi'], button_labels=['Yes', 'No'], fontsize=header_fontsize)
                buttons = window.get_buttons()
    ...
    def analysis_callback(self) -> None:
        ...
        if self.openfile_dropdown_value.get()=='Open folder':
            ...
            self.open_file(self.filelist[0])
            self.initialize()
            ShowInfo(message=' End of list', image=self.icons['check_circle'], button_labels=['OK'], fontsize=header_fontsize)
            ...
            if event.button == 1:
                ...
                if np.isfinite(self.datastack.vars[0].values[y, x]):
                    ...
                    for title, ylabel, ax_ in zip(titles, ylabels, axs):
                        ax_.set_xlabel('slice', fontsize=default_fontsize)
                        ax_.set_ylabel(ylabel, fontsize=default_fontsize)
                        ax_.set_xlim((1, self.datastack.nangle))
                        ax_.set_xticks(indx[::2], minor=True)
                        ax_.set_title(title, fontsize=header_fontsize)
    ...
    def slice4polar(self, stack:Stack, str_order:str) -> Stack:
        if hasattr(self, 'registration'):
            ...
            ShowInfo(message=' No registration', image=self.icons['blur_circular'], button_labels=['OK'], fontsize=header_fontsize)
            self.method.set('1PF')
