...
Source files for the Matlab App Designer app PolarimetryPlus (notably the `.mlapp` file). Additional files for post-processing polarimetry data are provided in the folder `AdditionalFiles`. For installing the app on your computer, check the latest releases on this page, download PolarPlusInstaller_*your_OS* and install the app.
